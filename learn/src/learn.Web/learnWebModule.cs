...
public class learnWebModule : AbpModule
{
    ...
    private void ConfigureVirtualFileSystem(IWebHostEnvironment hostingEnvironment)
    {
        if (hostingEnvironment.IsDevelopment())
        {
            ...
            {
                options.FileSets.ReplaceEmbeddedByPhysical<learnDomainSharedModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}learn.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<learnApplicationContractsModule>(Path.Combine(hostingEnvironment.ContentRootPath, $"..{Path.DirectorySeparatorChar}learn.Application.Contracts"));
    ...
    private void ConfigureNavigationServices(IConfiguration configuration)
    {
        ...
        {
            options.MenuContributors.Add(new learnMenuContributor(configuration));
        });

        Configure<AbpToolbarOptions>(options =>
        {
            options.Contributors.Add(new learnToolbarContributor());
        });
    }
    ...
    {
        var dataProtectionBuilder = context.Services.AddDataProtection().SetApplicationName("learn");
        if (!hostingEnvironment.IsDevelopment())
        {
            var redis = ConnectionMultiplexer.Connect(configuration["Redis:Configuration"]!);
            dataProtectionBuilder.PersistKeysToStackExchangeRedis(redis, "learn-Protection-Keys");
        }
    }
